...
2. 填入你的 **Base URL**（OpenAI 兼容接口地址）、**API Key** 以及你想使用的默认模型。也可以从供应商下拉选择（按国内 / 国际 / 网关分组），会自动带上推荐地址、协议与鉴权方式；Azure / Cloudflare / Bedrock 等用占位符表单填资源名或区域，不必手改 URL。
3. **协议**可选「自动（按模型选择原生协议）」：Claude → Anthropic Messages，Gemini → generateContent，GPT/o 系列在官方/Azure 上走 Responses。显式钉住某一协议后不会被改写。实际选用的协议、思考深度夹紧和缓存策略会显示在预设预览和生成 SSE 元数据里。
4. **缓存策略**：按厂商写法配置显式/隐式/尽力/关闭。设置里有近全屏「缓存原理」教学。消息气泡会显示缓存命中/写入。
5. 聊天输入栏的**模型控制面板**可切换模型、思考深度（含关闭思考）和 Fast 模式（约 2× 计费）。深度 `none` 会在各协议上真正关掉思考。
6. 保存设置。这些信息会存储在 `data/settings.json` 中。  
...
- **统一体验系统**：`v0.600` 起统一 Surface/Card/Button/Input/Modal/Drawer 视觉语言，主要按钮、表单、弹窗、抽屉、聊天气泡、设置面板与浮层使用同一套 token、焦点环、层级和 reduced-motion 规则。
