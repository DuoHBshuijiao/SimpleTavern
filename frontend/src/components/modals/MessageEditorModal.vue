    ...
    <div v-if="show" class="modal">
      ...
      <div class="modal-content chat-modal-width-700-92 glass-panel-floating border-white/10 shadow-2xl">
          ...
          <div class="space-y-6">
            <div class="form-group">
              ...
              <div class="flex flex-wrap gap-3">
                <div 
                  ...
                  :class="messageRole === 'system' ? 'border-brand bg-brand/20 shadow-brand/20 shadow-sm' : 'border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20'"
                  @click="emit('update:messageRole', 'system')"
                >
                  <Settings class="w-4 h-4" :class="messageRole === 'system' ? 'text-brand-300' : 'text-gray-400'" />
                  <span class="text-sm font-medium" :class="messageRole === 'system' ? 'text-brand-100' : 'text-gray-300'">系统</span>
                </div>
                <div 
                  class="cursor-pointer border rounded-xl p-2 px-4 flex items-center gap-2 transition-all duration-200"
                  :class="messageRole === 'assistant' ? 'border-brand bg-brand/20 shadow-brand/20 shadow-sm' : 'border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20'"
                  @click="emit('update:messageRole', 'assistant')"
                >
                  <ModernAvatar :src="characterAvatarUrl" :size="20" aspect="1" rounded="rounded-sm" />
                  <span class="text-sm font-medium" :class="messageRole === 'assistant' ? 'text-brand-100' : 'text-gray-300'">角色</span>
                </div>
                <div 
                  class="cursor-pointer border rounded-xl p-2 px-4 flex items-center gap-2 transition-all duration-200"
                  :class="messageRole === 'user' ? 'border-brand bg-brand/20 shadow-brand/20 shadow-sm' : 'border-white/10 bg-white/5 hover:bg-white/10 hover:border-white/20'"
                  @click="emit('update:messageRole', 'user')"
                >
                  <ModernAvatar :src="userAvatarUrl" :size="20" aspect="1" rounded="rounded-sm" />
                  <span class="text-sm font-medium" :class="messageRole === 'user' ? 'text-brand-100' : 'text-gray-300'">用户</span>
                </div>
              </div>
            </div>
            ...
            <div class="form-group">
              ...
              <textarea
                ...
                class="input textarea h-64 w-full rounded-xl p-4 bg-black/20 border border-white/10 focus:border-brand/50 text-gray-200 placeholder-gray-500/50 resize-none outline-none transition-all custom-scrollbar leading-relaxed"
        ...
        <div class="modal-footer p-4 border-t border-white/5 bg-black/20 flex justify-end gap-3">
          ...
          <button 
            class="px-4 py-2 rounded-xl text-sm font-medium bg-brand hover:bg-brand-hover text-white shadow-lg shadow-brand/20 border border-brand/20 transition-all flex items-center gap-2 whitespace-nowrap" 
