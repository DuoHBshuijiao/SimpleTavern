...
- [x] T-201 ~ T-208（composable、数据完整性、import warning、UI 首批）
- [x] T-209 UI/动画全面收束
...
- 仓库外黑盒测试（`docs/specs/`）；本仓库不引入 Playwright
...
- `T-106-tests-docs-version`：更新 README/CHANGELOG/state/version（当时补过 UI primitive / dialog / MVU 错误语义）。
...
- 为 modal/drawer 建立可复用组件。→ **v0.700 已建；仓库内组件测试已拆除**
