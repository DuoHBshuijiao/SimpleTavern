...
- T-804 已完成：ProviderAdapter 内核 + OpenAI-compatible 迁入适配器；`openai_compat` ABI 保留。
- 当前门禁：后端 226 tests。
...
- 下一批：T-805 原生协议（Responses / Anthropic / Gemini）。

## 后续进度（2026-09-20）

- T-805 / T-806 / T-807 已完成。
- T-808～T-813 已完成（定价/统计、Brave+原生联网、领域性能说明、orphan/锁超时、`useChatGeneration`、迁移警告）。
- 仓库内不跑自动化测试。剩余 T-814 仓库外黑盒核对。
