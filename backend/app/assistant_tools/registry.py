...
def _all_registered() -> list[RegisteredTool]:
    ...
    return [
        ...
        _fn(
            "kg_upsert_entity",
            "新建或按 name+type 合并知识图谱实体。参数：name、type（人物|地点|物品|势力|事件）、properties（可选键值对）。返回 entityId。",
            {
                "type": "object",
                "properties": {
                    "name": {"type": "string", "description": "实体显示名"},
                    "type": {
                        "type": "string",
                        "enum": ["人物", "地点", "物品", "势力", "事件"],
                        "description": "实体类型",
            ...
            needs_memory_write=False,
            needs_destructive=False,
            handler=H["kg_upsert_entity"],
            scopes=frozenset({CH}),
            risk=ToolRisk.WRITE,
        ),
        _fn(
            "kg_delete_entity",
            "逻辑删除知识图谱实体（保留溯源），并移除关联关系。参数：entity_id。",
            {
                "type": "object",
            ...
            needs_memory_write=False,
            needs_destructive=False,
            handler=H["kg_delete_entity"],
            scopes=frozenset({CH}),
            risk=ToolRisk.WRITE,
        ),
        _fn(
            "kg_upsert_relation",
            "新建或更新关系三元组。subject_id、object_id 为实体 ID（object 也可为字面量字符串）。predicate 自由拟定。confidence 可选 0~1。",
            {
                "type": "object",
                "properties": {
            ...
            needs_memory_write=False,
            needs_destructive=False,
            handler=H["kg_upsert_relation"],
            scopes=frozenset({CH}),
            risk=ToolRisk.WRITE,
        ),
        _fn(
            "kg_get_context",
            "返回当前会话知识图谱摘要文本，用于理解 RP 注入内容。",
            {"type": "object", "properties": {}, "additionalProperties": False},
            needs_chat=True,
            needs_memory_write=False,
            needs_destructive=False,
            handler=H["kg_get_context"],
            scopes=frozenset({CH}),
            risk=ToolRisk.READ,
        ),
        _fn(
            "kg_query",
            "按条件查询实体与关系。可选：entity_name、relation_type（谓语模糊）、type（实体类型）。",
            {
                "type": "object",
                "properties": {
