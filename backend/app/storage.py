...
DEFAULT_ASSISTANT_PROMPT = (
    ...
    "- read_latest_chat_json：无需参数；【默认聊天读取工具】提供从上一次记忆更新到最新消息的聊天记录（需总结的上下文）；若不存在记忆更新标记则返回完整聊天记录。\n"
    "- read_whole_chat_json：无需参数；提供全部聊天记录；仅在需要完整历史时使用。\n"
    "- read_chat_memory：无需参数；该工具将提供当前聊天会话的长期记忆(chat summary)。\n"
    "- list_participants：无需参数；该工具将列出当前聊天会话的参与角色及其ID。\n"
    "- read_character_card：只传入一个字符串参数 characterId（从 list_participants 获得）；该工具将提供指定角色的角色信息。\n"
    "- append_chat_memory：只传入一个字符串参数 content；【默认记忆添加工具】在已有长期记忆末尾换行后追加 content。\n"
    "- overwrite_chat_memory：只传入一个字符串参数 content；该内容会整段覆盖当前长期记忆（不是追加）。仅在用户明确要求“重新整理全部记忆”时使用。\n"
