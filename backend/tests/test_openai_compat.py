...
def test_list_models_maps_401_instead_of_returning_empty_list() -> None:
    ...
    async def run() -> AppError:
        with (
            patch("app.llm.providers.openai_compatible_chat.get_async_http_client", return_value=UnauthorizedClient()),
...
def test_nonstream_empty_choices_fast_fails() -> None:
    async def run() -> AppError:
        ...
        with (
            patch("app.llm.providers.openai_compatible_chat.get_async_http_client", return_value=client),
            patch("app.services.http_log._write_record", AsyncMock()),
        ):
            try:
